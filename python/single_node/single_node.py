...
class Tfconfig():
    ...
    def update(self, task_id):
        ...
        if self.data_mode == "all" or self.data_mode == "all_unknown":
            self.data_directory = "data"
        elif self.data_mode == "lnc":
        ...
        if self.data_dir_name[6] == "_" and self.data_dir_name[4] == "_":
...
def opt_trfmr(tfconfig):
    ...
    class CustomModel(tf.keras.Model):
        ...
        def test_step(self, data_combined, y=None, first_batch=None):
            ...
            if tfconfig.use_TAPE_feature == 1:
                tfconfig.p_tape_tf = self.flatten_n_batch(datalist[8])
            predictions, pweight, rweight, pself, rself, proout, rnaout = self.transformer(tfconfig)  # prediction, pweight, rweight, p_cr_weight, r_cr_weight
            loss = loss_function(tfconfig.label[0], predictions)
            loss_avg.update_state(loss)
            auc.update_state(tfconfig.label[0], predictions)
            tfconfig.batch_count += 1
    ...
    ctensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=train_log_dir, histogram_freq=1, write_graph=False)
    ...
    if tfconfig.training == 1:
        ...
        callbacks = [ctensorboard_callback, CustomCallback()]
